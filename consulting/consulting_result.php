  ...
  <div class="py-5 text-center parallax cover gradient-overlay" style="">
    <div class="container d-flex flex-column">
      <div class="row">
		...
		<!-- SHOWING RESULTS FROM KEYWORDS -->
		<?php
		  $sql = "SELECT question, id_faq FROM faq WHERE key_word LIKE '%".$_GET["keyword"]."%' ";
		  $num = 0;
		  $result = $conn->query($sql);
		  if ($result){
		    $num = $result->num_rows;
		    echo "<p align = right> Found <b><u>".$num."</u></b> Question(s) </p>";
		    if ($result->num_rows>0){
		      while($row = $result->fetch_assoc()){
		        echo "<div class=row>";
		        	echo "<div class=col>";
		  		 		echo "<h5 align = left>" .$row['question'];
		    			echo "</h5></div>";
		        	echo "<a href='question_detail.php?id=".$row['id_faq']."'>
		        	<button class='btn btn-primary' align = right >Open</button></a>";
		    	echo "</div>";
		      }
		    }
		    else{
		      echo "<p id ='desc'>Sorry. The keyword '".$_GET["keyword"]."' is not found.</p><br>";
		    }
		  }
		  else{
		    echo "<p id ='desc'>Sorry. The keyword '".$_GET["keyword"]."' is not found.</p><br>";
    ...
    <div class="row text-right">
          <div class="col-md-10" style=""><button class="btn mt-4 mb-3 rounded btn-lg btn-primary text-white" onclick="window.location = '../'"><b>Back to home</b></button> <button class="btn mt-4 mb-3 rounded btn-lg btn-primary text-white" onclick="window.location = 'input_question.php'"><b>Input question</b></button></div>
    </div>
